...
/*				var hd = '<w:p w:rsidR="00932176" w:rsidRPr="00997056" w:rsidRDefault="00932176" w:rsidP="00997056">'
				...
				+    '</w:p>';
*/
				var hd = wParagraph( {content:specifData.hierarchies[h].title,heading:1} ),
					ct = renderChildrenOf( specifData.hierarchies[h], 1 );
...
*/			}	
		...
		//		console.debug( 'statements', r.title, sts );
				...
				for( cid in sts ) {
					...
					if( sts[cid].subjects.length>0 ) {
						...
						row = wTableCell( cell );
						row += wTableCell( wParagraph( sTi ));
						row += wTableCell( wParagraph( titleOf( r, itemById(specifData[rClasses],r[rClass]), null, opts ) ));
						ct += wTableRow( row )
					};
				...
				for( a=0,A=r.properties.length; a<A; a++ ) {
					...
					if( r.properties[a].value
						&& opts.descriptionProperties.indexOf(rt)>-1 ) {
							c1 += wParagraph(valOf( r.properties[a] ))
		...
		//			console.debug('propVal',r.properties[a].value);
					...
					c2 += wTableRow( wTableCell( wParagraph(rt)) + wTableCell( wParagraph(valOf( r.properties[a] ))) )
						...
						function oxmlRow(str) {
							return str.replace(/<tr>([\s\S]*?)<\/tr>/g, function($1,$2) {
								$2 = $2.replace(/<th>([\s\S]*?)<\/th>/g, function($1,$2) {
										...
										return wTableCell( wParagraph( {content:$2, font:{weight:'bold'}} ))
										});
								$2 = $2.replace(/<td>([\s\S]*?)<\/td>/g, function($1,$2) {
										// the 'td' cell with it's content
										return wTableCell( wParagraph( $2 ) )
										});
...
//				console.debug('wParagraph',ct)
				...
				// else, a single text item with formatting option:
				return '<w:p>'+hd(ct.heading)+wRun(ct)+'</w:p>'
...
//						+		 '<w:bookmarkEnd w:id="bm-'+bookmarkId+'"/>'
			function wRun( ct ) {
				if( ct.bookmark ) {
					...
					return '<w:bookmarkStart w:id="'+bmId+'" w:name="_'+ct.bookmark+'"/><w:r>'+wText(ct)+'</w:r><w:bookmarkEnd w:id="'+bmId+'"/>'
				};
				if( ct.hyperlink ) 
					return '<w:hyperlink w:anchor="_'+ct.hyperlink+'"><w:r><w:rPr><w:rStyle w:val="Hyperlink"/></w:rPr>'+wText(ct)+'</w:r></w:hyperlink>';
				return '<w:r>'+(ct.font&&ct.font.weight=='bold'?'<w:rPr><w:b/></w:rPr>':'')+wText(ct)+'</w:r>'
			}
			function wText( ct ) {
				if( typeof(ct) == 'string' ) 
					return '<w:t>'+ct+'</w:t>';
				return '<w:t>'+ct.content+'</w:t>'
			}
