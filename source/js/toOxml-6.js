...
//					c2 += wTableRow( wTableCell( wParagraph(rt)) + wTableCell( wParagraph(valOf( r.properties[a] ))) )
					...
					c2 += wTableRow( wTableCell( wParagraph(rt)) + wTableCell( c1 ))
...
*/
		...
		//			console.debug('str',str);
			...
			//		if( typeof(opts.clickableElements)!='boolean' ) opts.clickableElements = false;
					...
					// Formatting information may be applied at both paragraphs and runs.
					...
					return blocks;
						...
						function splitParagraphs(bL,txt) {
							...
							do {
								...
								txt = txt.replace(/([\s\S]*?)(<p *\/>|<p[^>]*>[\s\S]*?<\/p>|<ul[^>]*>[\s\S]*?<\/ul>|<ol[^>]*>[\s\S]*?<\/ol>|<table[^>]*>[\s\S]*?<\/table>)/g, function($1,$2,$3) {
							...
							//			bL.push( {p:{content:$2}} );
									...
									// d) an unordered list:
									$3 = $3.replace(/<ul>([\s\S]*?)<\/ul>/g, function($1,$2) {
									...
									// e) an ordered list:
									$3 = $3.replace(/<ol>([\s\S]*?)<\/ol>/g, function($1,$2) {
										$2.replace(/<li>([\s\S]*?)<\/li>/g, function($1,$2) {
										...
										function parseRow(str) {
											return str.replace(/<tr>([\s\S]*?)<\/tr>/g, function($1,$2) {
												...
												$2 = $2.replace(/<th>([\s\S]*?)<\/th>/g, function($1,$2) {
														...
														cs.push( {p:{content:$2, font:{weight:'bold'}}} );
														return ''
														});
									...
									$3 = $3.replace(/<table[^>]*>([\s\S]*?)<\/table>/g, function($1,$2) {
...
//							console.debug('splitRuns',bl);
							...
							//		txt = txt.replace(/>/g, function ($0){ return '&gt;' });
									...
									// Find the next tag:
									txt = txt.replace(/([\s\S]*?)(<b>|<\/b>|<i>|<\/i>|<em>|<\/em>|<span[^>]*>|<\/span>)/g, function($0,$1,$2) {
										// store the preceding run with a clone of the current formatting:
										if( !isEmpty($1) )
											p.runs.push({content:$1,font:{weight:fmt.font.weight,style:fmt.font.style,color:fmt.font.color}});
//											p.runs.push({content:$1,font:fmt.font});

										// remove the next tag and update the formatting:
										$2 = $2.replace(/<b>/, function($0,$1) {
											fmt.font.weight = 'bold'
											return ''
										});
										$2 = $2.replace(/<\/b>/, function() {
											delete fmt.font.weight	// simply, since there is only one value so far.
											return ''
										});
										// assuming that <i> and <em> are not nested:
										$2 = $2.replace(/<i>|<em>/, function($0,$1) {
											fmt.font.style = 'italic'
											return ''
										});
										$2 = $2.replace(/<\/i>|<\/em>/, function() {
									...
									let r = {content:txt,font:fmt.font};
									p.runs.push(r);
									delete p.content;
									delete p.font;
									console.debug('splitRuns',r);
									return
								}
							if( bl.table ) {
...
/*					// Replace <object>, usually with images:
