...
//	if( !opts.linkFontColor ) opts.linkFontColor = '#005A92';	// darker
	...
	function image2base64(f,handle) {			
		const reader = new FileReader();
		reader.addEventListener('loadend',handle);
		reader.readAsDataURL(f.blob)
	}
	
// -----------------------
	function createOoxml() {
		// create the file content as OOXML:
		...
		//			console.debug('#',rC,pr,rClass);
					...
					switch( dT.type ) {
						case 'xs:enumeration':
							...
							return ct;
						case 'xhtml':
							return titleLinks( xhtml2ooxml( pr.value, opts ), opts )
						case 'xs:string':
