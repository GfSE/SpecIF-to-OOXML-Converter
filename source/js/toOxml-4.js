...
// -----------------------
	...
	//				headings: [],
			...
			for( var h=0,H=specifData.hierarchies.length; h<H; h++ ) {
				...
				var hd = '<w:p w:rsidR="00932176" w:rsidRPr="00997056" w:rsidRDefault="00932176" w:rsidP="00997056">'
				+        '<w:pPr>'
...
/*				// Section ID
				hd += '<w:p w:rsidR="002676EC" w:rsidRDefault="002676EC" w:rsidP="00997056">'
				+		'<w:pPr>'
				+			'<w:rPr>'
				+				'<w:lang w:val="en-US" />'
				+			'</w:rPr>'
				...
				+	'</w:p>';
*/
				var ct = renderChildrenOf( specifData.hierarchies[h], 1 );

				oxml.sections.push( hd+ct )
			};
	...
	*/		
		...
		//		console.debug( 'statements', r.title, sts );
				...
				// the heading:
//				let ct = '<w:p w:rsidR="00BC2601" w:rsidRPr="00E5017E" w:rsidRDefault="00E5017E" w:rsidP="00E5017E"><w:r><w:t>'+opts.statementsLabel+'</w:t></w:r></w:p>',
//				let ct = wParagraph( {content: opts.statementsLabel, heading: 3} ),
				let ct = wParagraph( opts.statementsLabel ),
					sTi, row, cell;
				...
				for( cid in sts ) {
					...
					if( sts[cid].subjects.length>0 ) {
						...
						for( i=0, I=sts[cid].subjects.length; i<I; i++ ) {
							...
							cell += wParagraph( titleOf( r2, itemById( specifData[rClasses], r2[rClass]), null, opts ), {hyperlink:anchorOf( r2 )})
						};
						row = wTc( cell );
						row += wTc( wParagraph( sTi ));
						row += wTc( wParagraph( titleOf( r, itemById(specifData[rClasses],r[rClass]), null, opts ) ));
						ct += wTr( row )
					};
...
/*						function getType( str ) {
		...
		//					oxml.imageLinks.push({ref:imageIDcount,id:u2.replace('\\','/'),type:extOf(u2)});
							...
							return wPict(imgCnt,$4,'200pt','100pt')
						}
					);
...
//					txt = txt.replace(/<p *\/>/g,'<w:p w:rsidRDefault="00F717F9" w:rsidP="00F717F9"><w:pPr><w:rPr><w:lang w:val="en-US" /></w:rPr></w:pPr><w:r><w:rPr><w:lang w:val="en-US" /></w:rPr><w:t/></w:r></w:p>');
					txt = txt.replace(/<p *\/>/g,wParagraph(''));
					txt = txt.replace(/<p[^>]*>([\s\S]*?)<\/p>/g, function($1,$2) {
						return wParagraph( $2 )
//						return '<w:p w:rsidRDefault="00F717C9" w:rsidP="00F717F9"><w:pPr><w:rPr><w:lang w:val="en-US" /></w:rPr></w:pPr><w:r><w:rPr><w:lang w:val="en-US" /></w:rPr><w:t>'
//								+ $2
//								+ '</w:t></w:r></w:p>'
					});
					
					// Transform lists:
					txt = txt.replace(/<ul>([\s\S]*?)<\/ul>/g, function($1,$2) {
						return $2.replace(/<li>([\s\S]*?)<\/li>/g, function($1,$2) {
							return '<w:p w:rsidR="000C0D2D" w:rsidRPr="00335752" w:rsidRDefault="00FB4B48" w:rsidP="00FB4B48"><w:pPr><w:pStyle w:val="Listenabsatz"/><w:numPr><w:ilvl w:val="0"/><w:numId w:val="1"/></w:numPr><w:rPr><w:lang w:val="en-US"/></w:rPr></w:pPr><w:bookmarkStart w:id="0" w:name="_GoBack"/><w:r w:rsidRPr="00335752"><w:rPr><w:lang w:val="en-US"/></w:rPr><w:t>'
									+ $2
									+ '</w:t></w:r></w:p>'
						})
					});
					txt = txt.replace(/<ol>([\s\S]*?)<\/ol>/g, function($1,$2) {
						return $2.replace(/<li>([\s\S]*?)<\/li>/g, function($1,$2) {
							// ToDo: use correct replacement for ordered lists:
							return '<w:p w:rsidR="000C0D2D" w:rsidRPr="00335752" w:rsidRDefault="00FB4B48" w:rsidP="00FB4B48"><w:pPr><w:pStyle w:val="Listenabsatz"/><w:numPr><w:ilvl w:val="0"/><w:numId w:val="1"/></w:numPr><w:rPr><w:lang w:val="en-US"/></w:rPr></w:pPr><w:bookmarkStart w:id="0" w:name="_GoBack"/><w:r w:rsidRPr="00335752"><w:rPr><w:lang w:val="en-US"/></w:rPr><w:t>'
									+ $2
									+ '</w:t></w:r></w:p>'
						})
					...
					// abschließende Ersetzungen 
					// Dopplungen entfernen
	//				ct = ct.replace(/<w:p w:rsidRDefault="00F717C9" w:rsidP="00F717F9">[\s]*<w:pPr>[\s]*<w:rPr>[\s]*<w:lang w:val="en-US" \/>[\s]*<\/w:rPr>[\s]*<\/w:pPr>[\s]*<w:r>[\s]*<w:rPr>[\s]*<w:lang w:val="en-US" \/>[\s]*<\/w:rPr>[\s]*<w:t>[\s]*<w:p w:rsidR="00BB24CF" w:rsidRDefault="00BB24CF">[\s]*<w:r>[\s]*<w:pict>/g,'<w:p w:rsidR="00BB24CF" w:rsidRDefault="00BB24CF"><w:r><w:pict>');
	//				ct = ct.replace(/<\/w:pict>[\s]*<\/w:r>[\s]*<\/w:p>[\s]*<\/w:t>[\s]*<\/w:r>[\s]*<\/w:p>/g,'</w:pict></w:r></w:p>');
	//				ct = ct.replace(/<w:p w:rsidR="00BC2601" w:rsidRPr="00E5017E" w:rsidRDefault="00E5017E" w:rsidP="00E5017E">[\s]*<\/w:t>/g,'</w:t>');
		...
		//			console.debug('#',rC,pr,rClass);
					...
					switch( dT.type ) {
						case 'xs:enumeration':
							...
							return ct;
						case 'xhtml':
							return titleLinks( xhtml2oxml( pr.value, opts ), opts )
						case 'xs:string':
			...
			function wParagraph( ct, opts ) {
				if( typeof opts != 'object' ) opts = {};
				if( typeof opts.font != 'object' ) opts.font = {};
//				if( typeof opts.font.weight != 'string' ) opts.font.weight = 'normal';
				ct = opts.hyperlink? wHyperlink(ct,opts.hyperlink) : wRun(ct, opts); 
//				console.debug('wParagraph',ct)
				return '<w:p>'+ct+'</w:p>'
			}
			function wRun( ct, opts ) {
				return '<w:r>'+(opts.font.weight=='bold'?'<w:rPr><w:b/></w:rPr>':'')+wText(ct)+'</w:r>'
			}
			function wText( ct ) {
				return '<w:t>'+ct+'</w:t>'
			}
			function wHyperlink( lb, an ) {
				// inserts a hyperlink at 'run' level:
				return '<w:hyperlink w:anchor="_'+an+'"><w:r><w:rPr><w:rStyle w:val="Hyperlink"/></w:rPr><w:t>'+lb+'</w:t></w:r></w:hyperlink>'
			}
			function wPict( idx, t, w, h ) {
...
//					+	'</w:p>'
			...
			function wTc( ct ) {
				return '<w:tc>'
					...
					+		'<w:tcBorders>'
					+			'<w:top w:val="single" w:sz="3" w:space="0" w:color="DDDDDD"/>'
					+			'<w:left w:val="single" w:sz="3" w:space="0" w:color="DDDDDD"/>'
					+			'<w:bottom w:val="single" w:sz="3" w:space="0" w:color="DDDDDD"/>'
					+			'<w:right w:val="single" w:sz="3" w:space="0" w:color="DDDDDD"/>'
					+		'</w:tcBorders>'
//					+		'<w:tcMar>'
